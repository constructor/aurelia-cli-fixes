...
const logger = require('aurelia-logging').getLogger('TemplateClone');
...
module.exports = class {
  ...
  async execute(context) {
    this.templateProcessor = require('../../commands/template/template-processor')(context.state);
      ...
      try {
        if (this.hasGit()) {
          ...
          exec(cmd, (error, stdout, stderr) => {
            ...
            del([remove]).then(async paths => {
              this.ui.log('Preparing files...');
              await tc.templateProcessor.process(this.ui);
              resolve(true);
            });
          });
        }
  ...
  installDependencies(ui, state, dependencies) {
    ...
    var workingDirectory = process.cwd() + '\\Project';

    if (output)
      workingDirectory = process.cwd() + '\\' + output;
